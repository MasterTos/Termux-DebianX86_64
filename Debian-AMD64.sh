...
if [ "$first" != 1 ];then
	if [ ! -f $tarball ]; then
		...
		aarch64)
			archurl="amd64";
			wget https://github.com/MasterTos/Termux-DebianX86_64/raw/master/arm64/qemu-x86_64-static;
			chmod 777 qemu-x86_64-static;
			mv qemu-x86_64-static ~/../usr/bin ;;
		arm)
			archurl="amd64";
			wget https://github.com/MasterTos/Termux-DebianX86_64/raw/master/arm/qemu-x86_64-static;
			chmod 777 qemu-x86_64-static;
			mv qemu-x86_64-static ~/../usr/bin/ ;;
		amd64)
			archurl="amd64" ;;
...
command+=" -b /dev"
command+=" -b /proc"
command+=" -b debian-fs64/root:/dev/shm"
